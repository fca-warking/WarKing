...
const login = require('./WarKing/hzi');
...
global.anti = resolve(process.cwd(),'./WarKing/anti.json');
...
var configValue;
try {
    global.client.configPath = join(global.client.mainPath, "./config/config.json");
    ...
    else return logger.loader("Không Thể Tìm Thấy config.json", "error");
}
...
const { Sequelize, sequelize } = require("./WarKing/database");
...
const langFile = (readFileSync(`${__dirname}/WarKing/languages/${global.config.language || "en"}.lang`, { encoding: 'utf-8' })).split(/\r?\n|\r/);
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "./config/appstate.json"));
    ...
    login(loginData, async(loginError, loginApiData) => {
      ...
      global.client.api = loginApiData
        ...
        global.client.timeStart = new Date().getTime(),
            function () {
                const listCommand = readdirSync(global.client.mainPath + '/war/warking').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
                for (const command of listCommand) {
                    try {
                        var module = require(global.client.mainPath + '/war/warking/' + command);
            ...
            function() {
                const events = readdirSync(global.client.mainPath + '/war/events').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
                for (const ev of events) {
                    try {
                        var event = require(global.client.mainPath + '/war/events/' + ev);
       ...
       // logger.loader('=== ' + (Date.now() - global.client.timeStart) + 'ms ===')
        ...
        const listener = require('./WarKing/listen')(listenerData);
...
(async() => {
    try {
        ...
        authentication.sequelize = sequelize;
        const models = require('./WarKing/database/model')(authentication);

        const botData = {};
        botData.models = models
        onBot(botData);
    }
    catch (error) { logger(global.getText('mirai', 'successConnectDatabase', JSON.stringify(error)), '[ WWRKING ] '); }
