...
module.exports = {
    Normal: function(/** @type {string} */ Str, /** @type {() => any} */ Data, /** @type {() => void} */ Callback) {
        if (isHexcolor(global.Fca.Require.FastConfig.MainColor) != true) {
            ...
            const prefix = chalk.hex(global.Fca.Require.FastConfig.MainColor).bold(`${global.Fca.Require.FastConfig.MainName || '[ FCA-WARKING ]'} > `);
    ...
    Warning: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        ...
        console.log(chalk.magenta.bold('[ FCA-WARKING ] > ') + gradientMessage);
        if (getType(callback) == 'Function' || getType(callback) == 'AsyncFunction') {
            callback();
        }
        else return callback;
    ...
    Error: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        if (!str) {
            console.log(chalk.magenta.bold('[ WARKING - ERORR ] > ') + createGradientText("Already Faulty, Please Contact: Facebook.com/Lazic.Kanzu", '#FF0000', '#8B0000'));
        }
        const gradientMessage = createGradientText(String(str), '#FF0000', '#8B0000'); // Gradient từ đỏ sáng sang đỏ đậm
        console.log(chalk.magenta.bold('[ WARKING-ERROR ] > ') + gradientMessage);
        if (getType(callback) == 'Function' || getType(callback) == 'AsyncFunction') {
            callback();
        }
        else return callback;
    ...
    Success: function(/** @type {unknown} */ str, /** @type {() => void} */ callback) {
        ...
        console.log(chalk.hex('#9900FF').bold(`${global.Fca.Require.FastConfig.MainName || '[ FCA-WARKING ]'} > `) + gradientMessage);
        if (getType(callback) == 'Function' || getType(callback) == 'AsyncFunction') {
            callback();
        }
        else return callback;
